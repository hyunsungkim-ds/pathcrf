...
class StaticDenseCRF(nn.Module):
    """
    Dense CRF over K edge-states with static transition parameters.
    All transitions are allowed; transition scores are learned as a full KxK matrix.
    ...
    def __init__(
        ...
        # transitions[curr, prev]
        self.transitions = nn.Parameter(torch.zeros(self.K, self.K))
    ...
    def _compute_logZ(self, emissions: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        for t in range(1, T):
            ...
            score = torch.logsumexp(alpha.unsqueeze(1) + self.transitions.unsqueeze(0), dim=2)  # (B, K_curr)
    ...
    def _compute_gold_score(
        ...
        for t in range(1, T):
            ...
            trans_score = self.transitions[curr, prev]  # (B,)
    ...
    def decode(
        ...
        for t in range(1, T):
            emit_score = emissions[:, t]  # (B, K_curr)
            score = delta.unsqueeze(1) + self.transitions.unsqueeze(0)  # (B, K_curr, K_prev)
            best_score, best_prev = score.max(dim=2)  # (B, K_curr), (B, K_curr)
            new_delta = best_score + emit_score  # (B, K_curr)
    ...
    def dump_gold_score(
        ...
        for t in range(1, emissions.shape[1]):
            ...
            trans_score = self.transitions[curr, prev].item()
