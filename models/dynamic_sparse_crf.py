...
class DynamicSparseCRF(nn.Module):
    """
    Sparse CRF over K edge-states with edge embedding-based transition scoring.
    ...
    def __init__(
        ...
        self.register_buffer("inc_type", inc_type)  # (K, M) long, -1 padded (unused here)
    ...
    def _compute_logZ(
        ...
        inc_idx_edge = inc_idx.view(1, K, -1, 1).expand(B, -1, -1, D)  # (B, K, M, D)
        ...
        for t in range(1, T):
            ...
            prev_alpha = prev_alpha.gather(2, inc_idx_alpha)  # (B, K_curr, M)

            prev_embed = edge_embeds[:, t - 1]  # (B, K, D) = (B, K_prev, D)
            curr_embed = edge_embeds[:, t]  # (B, K, D) = (B, K_curr, D)
            prev_embed = prev_embed.unsqueeze(1).expand(-1, K, -1, -1)  # (B, K_curr, K_prev, D)
            curr_embed = curr_embed.unsqueeze(2).expand(-1, -1, inc_idx.size(1), -1)  # (B, K_curr, M, D)

            # Converting edge embedding pairs to transition scores
            prev_embed_gather = prev_embed.gather(2, inc_idx_edge)  # (B, K, M, D)
            trans_score = self.linear(torch.cat([prev_embed_gather, curr_embed], dim=-1)).squeeze(-1)  # (B, K, M)
    ...
    def _compute_gold_score(
        ...
        for t in range(1, T):
            ...
            is_allowed = ((inc_idx_t == prev.unsqueeze(1)) & inc_mask_t).any(dim=1)  # (B,)
            trans_score = torch.where(is_allowed, trans_score, forbid_score)  # (B,)
    ...
    def decode(
        ...
        """
        Viterbi decode using incoming lists and MLP transitions.
        """
        B, T, K = emissions.shape
        D = self.edge_embed_dim
        if mask is None:
        ...
        for t in range(1, T):
            ...
            prev_delta = prev_delta.gather(2, inc_idx_delta)  # (B, K_curr, M)
    ...
    def dump_gold_score(
        ...
        for t in range(1, emissions.shape[1]):
            ...
            # allowed check via incoming list
            inc_idx_t = self.inc_idx[curr]  # (M,)
            inc_mask_t = self.inc_mask[curr]
            allowed = ((inc_idx_t == prev) & inc_mask_t).any().item()
